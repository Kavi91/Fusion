...
# Alternative absolute import (use if relative import fails after adding __init__.py)
from lorcon_lo.utils.process_data import count_seq_sizes
from lorcon_lo.utils.common import euler_to_rot_mat
...
def plot_gt(Y_origin_data, pose_folder, preprocessed_folder, data_seqs, seq_sizes, dim="2d", save_graph=True, dataset="KITTI"):
    ...
    for seq in data_seqs:
        ...
        for idx, row in enumerate(Y_origin_data[start_idx:end_idx]):
            # Assuming row is a string with space-separated values
