...
class FusionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Compute relative poses (translation and yaw)
        translations = poses[1:, :3] - poses[:-1, :3]  # Shape: [seq_len-1, 3]
        quaternions = poses[:, 3:]  # Shape: [seq_len, 4]
        # Normalize quaternions
        quaternions = F.normalize(quaternions, p=2, dim=-1)
        # Extract yaw from quaternions (w, x, y, z)
        w = quaternions[:, 0]
        x = quaternions[:, 1]
        y = quaternions[:, 2]
        z = quaternions[:, 3]
        yaws = torch.atan2(2 * (w * z + x * y), 1 - 2 * (y * y + z * z))  # Shape: [seq_len]
        # Compute relative yaw (delta phi)
        relative_yaws = yaws[1:] - yaws[:-1]  # Shape: [seq_len-1]
        # Handle wrap-around
        relative_yaws = torch.atan2(torch.sin(relative_yaws), torch.cos(relative_yaws))
        relative_poses = torch.cat([translations, relative_yaws.unsqueeze(-1)], dim=-1)  # Shape: [seq_len-1, 4]
