...
for epoch in range(total_epochs):
    ...
    for batch_idx, batch in enumerate(train_bar):
        ...
        loss, L_t, L_rot = criterion(outputs, targets)
        loss.backward()
        ...
        train_loss_rot += L_rot.item()
        train_bar.set_postfix({"train_loss": loss.item(), "train_loss_t": L_t.item(), "train_loss_rot": L_rot.item()})
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_bar):
            ...
            val_bar.set_postfix({"val_loss": loss.item(), "val_loss_t": L_t.item(), "val_loss_rot": L_rot.item()})
    
    val_loss /= len(val_loader)
    val_loss_t /= len(val_loader)
    val_loss_rot /= len(val_loader)
    ...
    wandb.log({
        ...
        "train_loss_rot": train_loss_rot,
        "val_loss": val_loss,
        "val_loss_t": val_loss_t,
        "val_loss_rot": val_loss_rot,
        "val_ate": val_ate,
        "val_rpe_trans": val_rpe_trans,
        "val_rpe_rot": val_rpe_rot,
    })
    print(f"Epoch {epoch+1}: Train Loss: {train_loss:.4f} (T: {train_loss_t:.4f}, Rot: {train_loss_rot:.4f}), "
          f"Val Loss: {val_loss:.4f} (T: {val_loss_t:.4f}, Rot: {val_loss_rot:.4f}), "
    ...
    if (epoch + 1) % 10 == 0:
        torch.save(model.state_dict(), f"models/fusion_model_epoch_{epoch+1}.pth")
